...
Feature: Bank Account setting
  ...
  @jira.QANOV-204530 @android @blaude @ios @jira.cv.13.9 @jira.link.depends_on.QANOV-204367 @jira.link.detects.OBO2DE-208
  ...
  Scenario: Postpay user can see the "bank account" setting in the Bills screen when does not have bank account
    ...
    Given user does not have bank account
     ...
     Then the "bank_account_entrypoint" entrypoint with "Bankverbindung" text is displayed
      ...
      And the "bills_settings.bank_account_chevron" icon is displayed

  @jira.QANOV-204531 @android @blaude @ios @jira.cv.13.9 @jira.link.depends_on.QANOV-204367 @automatic @mobile
  @regression @next @live @webapp
  Scenario: Postpay user can see the "bank account" setting in the Bills screen when it's activated
    ...
    Given user has "bank_account" service in "activated" status
      And user is in the "Bills" page
     Then the "bank_account_title" entrypoint with "Bankverbindung" text is displayed
      And the "bank_account_description" textfield with the "^DE\*{16}\d{4}$" format is displayed
      And the "bank_account_tag" textfield is not displayed
      And the "bank_account_chevron" icon is displayed

  @jira.QANOV-532737 @android @blaude @ios @jira.cv.24.0 @jira.link.depends_on.QANOV-204367 @manual @mobile @regression
  ...
  @jira.QANOV-204532 @smoke @android @automatic @blaude @ios @jira.cv.13.9 @jira.link.depends_on.QANOV-204530
  @mobile @raitt_drop3 @webapp @qa
  ...
  @jira.QANOV-204533 @sanity @android @automatic @ber @blaude @ios @jira.cv.13.9 @jira.link.depends_on.QANOV-204531
  @mobile @raitt_drop3 @webapp @live @next
