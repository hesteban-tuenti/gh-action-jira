# -*- coding: utf-8 -*-
@jira.QANOV-499435 @not_hardening
Feature: Card management - Naked cards stepper flow


  @jira.<jira_id> @<product> @jira.cv.<cv> @manual @web
  Scenario Outline: A user can access to the first screen in the stepper flow when creating a new naked cards card
    ...
    O2es will have naked cards enabled but will only make use of featured content, that's why the outline is regressed unlike the others.
    Given a "naked cards" module has been created in CMS
     When I go to "[CONF:cms.base][CONF:cms.modules_repo]" web page
      And the "Modules Repository" page is loaded
      And I click on the "edit_naked_cards_button" button
    ...
    Examples:
          | product | cv   | jira_id      |
          | vivobr  | 14.8 | QANOV-499436 |
          | o2uk    | 14.8 | QANOV-499437 |
          | moves   | 14.8 | QANOV-499438 |
          | blaude  | 14.8 | QANOV-499439 |
          | o2de    | 14.8 | QANOV-499440 |

    @regression
    Examples:
          | product | cv     | jira_id      |
          | o2es    | Future | QANOV-499441 |

  @jira.<jira_id> @<product> @jira.cv.<cv> @manual @regression @web
  Scenario Outline: A user can see the fields for a image slot of image asset circle in the first screen in the stepper flow when creating a new naked cards card
    WARNING: the paths defined in this test correspond to the PROD environment.
    However, this test must be executed in QA environment, so be sure to use the proper path.
    O2es will have naked cards enabled but will only make use of featured content, that's why the outline is regressed unlike the others.
    Given a "naked cards" module has been created in CMS
    ...
    Examples:
          | product | cv     | jira_id      |
          | vivobr  | 14.8   | QANOV-499442 |
          | o2uk    | 14.8   | QANOV-499443 |
          | moves   | 14.8   | QANOV-499444 |
          | blaude  | 14.8   | QANOV-499445 |
          | o2de    | 14.8   | QANOV-499446 |
          | o2es    | Future | QANOV-499447 |

  @jira.<jira_id> @<product> @jira.cv.<cv> @manual @regression @web
  Scenario Outline: A user can see the fields for a text slot of image asset circle in the first screen in the stepper flow when creating a new naked cards card
    WARNING: the paths defined in this test correspond to the PROD environment.
    However, this test must be executed in QA environment, so be sure to use the proper path.
    O2es will have naked cards enabled but will only make use of featured content, that's why the outline is regressed unlike the others.
    Given a "naked cards" module has been created in CMS
    ...
    Examples:
          | product | cv     | jira_id      |
          | vivobr  | 14.8   | QANOV-499448 |
          | o2uk    | 14.8   | QANOV-499449 |
          | moves   | 14.8   | QANOV-499450 |
          | blaude  | 14.8   | QANOV-499451 |
          | o2de    | 14.8   | QANOV-499452 |
          | o2es    | Future | QANOV-499453 |

  @jira.<jira_id> @<product> @jira.cv.<cv> @manual @web
  Scenario Outline: A user can access to the second screen in the stepper flow when creating a new naked cards card
    WARNING: the paths defined in this test correspond to the PROD environment.
    However, this test must be executed in QA environment, so be sure to use the proper path.
    O2es will have naked cards enabled but will only make use of featured content, that's why the outline is regressed unlike the others.
    Given a "naked cards" module has been created in CMS
    ...
    Examples:
          | product | cv   | jira_id      |
          | vivobr  | 14.8 | QANOV-499454 |
          | o2uk    | 14.8 | QANOV-499455 |
          | moves   | 14.8 | QANOV-499456 |
          | blaude  | 14.8 | QANOV-499457 |
          | o2de    | 14.8 | QANOV-499458 |

    @regression
    Examples:
          | product | cv     | jira_id      |
          | o2es    | Future | QANOV-499459 |

  @jira.<jira_id> @<product> @jira.cv.<cv> @manual @web
  Scenario Outline: A user can access to the third screen in the stepper flow when creating a new naked cards card
    WARNING: the paths defined in this test correspond to the PROD environment.
    However, this test must be executed in QA environment, so be sure to use the proper path.
    O2es will have naked cards enabled but will only make use of featured content, that's why the outline is regressed unlike the others.
    Given a "naked cards" module has been created in CMS
     When I go to "[CONF:cms.base][CONF:cms.modules_repo]" web page
      And the "Modules Repository" page is loaded
      And I click on the "edit_naked_cards" button
      And the "Edit Module" page is loaded
    ...
    Examples:
          | product | cv   | jira_id      |
          | vivobr  | 14.8 | QANOV-499460 |
          | o2uk    | 14.8 | QANOV-499461 |
          | moves   | 14.8 | QANOV-499462 |
          | blaude  | 14.8 | QANOV-499463 |
          | o2de    | 14.8 | QANOV-499464 |

    @regression
    Examples:
          | product | cv     | jira_id      |
          | o2es    | Future | QANOV-499465 |

  @jira.<jira_id> @<product> @jira.cv.<cv> @manual @web
  Scenario Outline: A user can create a new naked card uploading a new image
    ...
    Given a "naked cards" module has been created in CMS
     ...
     Then the "Edit Module" page is loaded
      And the "cards_title_list" elements list contains the text "qa test NakedCard"

    @smoke
    Examples:
          | product | cv   | jira_id      |
          | vivobr  | 14.8 | QANOV-499466 |
          | o2uk    | 14.8 | QANOV-499467 |
          | moves   | 14.8 | QANOV-499468 |
          | blaude  | 14.8 | QANOV-499469 |
          | o2de    | 14.8 | QANOV-499470 |

    @regression
    Examples:
          | product | cv     | jira_id      |
          | o2es    | Future | QANOV-499471 |

  @jira.<jira_id> @<product> @ber @jira.cv.Future @jira.link.parent_test_plan.QANOV-343179 @manual @sanity @web
  Scenario Outline: A user can configure naked cards card with a heart icon
    WARNING: the paths defined in this test correspond to the PROD environment.
    ...
    Given a "naked cards" module has been created in CMS
     ...
     When I go to "[CONF:cms.base][CONF:cms.modules_repo]" web page
      ...
      And I click on the "next_button" button
      And the "Card Actions" page is loaded
      And I fill in the "url_text_placeholder" field with "Qa Test"
      And I fill in the "url_link_placeholder" field with "https://www.google.es"
